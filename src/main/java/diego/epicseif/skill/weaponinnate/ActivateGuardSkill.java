package diego.epicseif.skill.weaponinnate;
...
public class ActivateGuardSkill extends WeaponInnateSkill {
    ...
    public void onInitiate(SkillContainer container) {
        ...
        container.getExecuter().getEventListener().addEventListener(EventType.HURT_EVENT_PRE, EVENT_UUID, (event) -> {
            ...
            if (event.getAmount() > 0.0F && container.isActivated() && !container.isDisabled() && phaseLevel > 0 && phaseLevel < 3 &&
                ...
                if (isFront) {
                    event.getPlayerPatch().playSound(EpicFightSounds.CLASH.get(), 0.0F, 0.0F);
    ...
    public void executeOnServer(ServerPlayerPatch executer, FriendlyByteBuf args) {
        executer.playSound(SoundEvents.PLAYER_BREATH, 0.0F, 0.0F);;
        ...
        if (executer.getSkill(this).isActivated()) {
            ...
            executer.playAnimationSynchronized(SeifAnimations.SEIF_INNER_ACTIVE, 0.0F);
        }
    }
    ...
    public List<Component> getTooltipOnItem(ItemStack itemstack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        list.add(Component.translatable(traslatableText).withStyle(ChatFormatting.WHITE).append(Component.literal(String.format("[%.0f]", this.consumption)).withStyle(ChatFormatting.RED)));
        list.add(Component.translatable(traslatableText + ".tooltip", tooltipArgs.toArray(new Object[0])).withStyle(ChatFormatting.BLACK));
